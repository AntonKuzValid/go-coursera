...
func main() {
	...
	fmt.Println("\nTGet call #1")
	posts := RSS{}
	err := TGet(mkey, 30, &posts, rebuild)
	fmt.Println("#1", len(Items), "err:", err)

	fmt.Println("\nTGet call #2")
	posts = RSS{}
	err = TGet(mkey, 30, &posts, rebuild)
	fmt.Println("#2", len(Items), "err:", err)
	...
	go func() {
		...
		posts = RSS{}
		err = TGet(mkey, 30, &posts, rebuild)
		fmt.Println("#async", len(Items), "err:", err)
	}()

	fmt.Println("\nTGet call #3")
	posts = RSS{}
	err = TGet(mkey, 30, &posts, rebuild)
	fmt.Println("#3", len(Items), "err:", err)
}
